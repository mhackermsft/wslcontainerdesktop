...
public sealed class FoundryLocalInstaller
{
    internal const string InstallScript = """
        ...
        $dependency = Get-AppxPackage -Name 'Microsoft.VCLibs.140.00.UWPDesktop' | Where-Object { $_.Architecture -eq 'X64' }
        if ($dependency | Where-Object { $_.Version.ToString() -ne $env:WSLCD_FOUNDRY_VCLIBS_VERSION }) { throw 'Different VCLibs dependency; installation blocked.' }
        Add-AppxPackage -Path $env:WSLCD_FOUNDRY_MSIX -DependencyPath $env:WSLCD_FOUNDRY_VCLIBS -ErrorAction Stop
        $installed = Get-AppxPackage -Name 'Microsoft.FoundryLocal' | Where-Object { $_.PackageFamilyName -eq 'Microsoft.FoundryLocal_8wekyb3d8bbwe' -and $_.Version.ToString() -eq $env:WSLCD_FOUNDRY_VERSION }
        if (-not $installed) { throw 'Installed version not confirmed.' }
        $resolved = $installed.Dependencies | Where-Object { $_.PackageFamilyName -eq 'Microsoft.VCLibs.140.00.UWPDesktop_8wekyb3d8bbwe' -and $_.Version.ToString() -eq $env:WSLCD_FOUNDRY_VCLIBS_VERSION }
        if (-not $resolved) { throw 'Exact dependency not confirmed.' }
        ...
        Func<string, CancellationToken, Task<bool>> confirm, Func<bool> isCurrent, IProgress<string>? progress, CancellationToken ct)
    {
        ...
        try
        {
            ...
            await using var dependency = await OpenVerifiedAsync(dependencyPath, package.VcLibs, ".appx", ct);
            ct.ThrowIfCancellationRequested();
            if (!isCurrent()) return new(FoundryLocalInstallState.Cancelled, "Original configuration changed; nothing installed.");
            progress?.Report("Awaiting explicit runtime-only installation consent; initial-model setup remains blocked…");
            if (!await confirm(package.Confirmation(original.Model), ct))
            ...
            return new(FoundryLocalInstallState.Installed, "Exact runtime package registered for this user. No server started or settings changed. Initial-model acquisition/load remains blocked.");
        }
